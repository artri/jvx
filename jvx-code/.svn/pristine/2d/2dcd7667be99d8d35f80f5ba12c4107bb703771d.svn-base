                                               ...
                                               implements ILinkedCellEditor
{
    ...
    protected String getDisplayValue(IDataRow pDataRow, String pColumnName) throws ModelException
    {
                ...
                && !(pDataRow instanceof IDataBook && ((IDataBook)pDataRow).getSelectedRow() < 0))
        {
            ...
            String[] columns = searchValues.keySet().toArray(new String[searchValues.size() + 1]);
            Object[] values = searchValues.values().toArray(new Object[searchValues.size() + 1]);
            
            columns[searchValues.size()] = refColumn;
            values[searchValues.size()] = value;
            ...
            if (lastPageForSearch == null 
                    ...
                    || (isMemDataBook && index != null 
                         && (refDataRow == null || !CommonUtil.equals(key, refKey) || !CommonUtil.equals(displayValue, getDisplayValueFromRow(refDataRow)))))
            {
