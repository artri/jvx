                                   ...
                                   implements IPushHandler
{
		...
		if (pSessionId != null)
		{
			...
			if (session != null)
			{
				...
				if (session.isInactive(lNow) || !session.isAlive(lNow))
				{
				    ...
				    log.info("Destroy invalid session: ", pSessionId, Boolean.valueOf(session.isInactive(lNow)), Boolean.valueOf(!session.isAlive(lNow)), 
									  ...
									  ChangedHashtable<String, Object> pProperties) throws Throwable
	{
	    ...
	    try
	    {
	        ...
	        try
	        {
    	        ...
    	        {
    	        	//no password logging
        			Map<String, Object> map = new HashMap<String, Object>(chtProps);
        			map.remove(IConnectionConstants.PASSWORD);
    	        	
    	        	recProp.setParameter(map);
    	        }
	        }
	        finally
	        {
    			...
    			if (session != null)
    			{
    				if (session.isInactive(lNow) || !session.isAlive(lNow))
    				{
		                ...
		                try
		                {
                            ...
                            log.info("Destroy invalid session: ", oId, ", ", Boolean.valueOf(session.isInactive(lNow)), 
                                      ", ", Boolean.valueOf(!session.isAlive(lNow)), ", ", Long.valueOf(session.getStartTime()));                 
	...
	private CacheMode getCacheMode(ISession pSession)
	{
	    ...
	    {
            ((SubSession)pSession).getMasterSession().setProperty(IConnectionConstants.PREFIX_SERVER + IConnectionConstants.PREFIX_SESSION + "cacheMode", cacheMode);
	    }

	    pSession.setProperty(IConnectionConstants.PREFIX_SERVER + IConnectionConstants.PREFIX_SESSION + "cacheMode", cacheMode);
