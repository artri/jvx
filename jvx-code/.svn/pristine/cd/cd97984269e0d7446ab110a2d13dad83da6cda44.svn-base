...
public class VTabsheet extends VTabsheetBase
            ...
            if (newStyleName != null && !newStyleName.isEmpty()) {
                if (!newStyleName.equals(styleName)) {
                    ...
                    if (styleName != null && !styleName.isEmpty()) {
                        ...
                        tabCaption.removeStyleName(CLASSNAME + "-caption-" + styleName);
                    }
                    // Set new style name
                    td.addClassName(TD_CLASSNAME + "-" + newStyleName);
                    tabCaption.addStyleName(CLASSNAME + "-caption-" + newStyleName);
     ...
     *
     * @param tabIndex of the tab to load
    ...
    /**
     * Clears all preferred size caches, as the tab content changed.
     */
    private void clearPreferredSizeCaches()
    {
        Connector parent = connector.getParent();
        
        while (parent instanceof ComponentConnector
...
//              && !(connector instanceof TabsheetConnector)           // preferred size is now supported
                && !(parent instanceof WindowConnector))
        {
            Element element = ((ComponentConnector)parent).getWidget().getElement();
            ...
            if (parent instanceof LayoutedPanelConnector)
            {
                LayoutedPanelConnector lpc = (LayoutedPanelConnector)parent;
                
                lpc.clearCaches();
                connector.getLayoutManager().setNeedsLayout(lpc);
            }
     ...
     * For internal use only. May be removed or replaced in the future.
     * 
     *  @return the content area border width
     */
    ...
    /**
     * Resets the tab view, and scrolls to the active tab, without gaining the focus.
     */
    ...
    public com.google.gwt.user.client.Element getSubPartElement(
        ...
        if ("tabpanel".equals(subPart)) {
            ...
            int tabIx = Integer.parseInt(result.getGroup(1));
