...
public class VScrollTable extends FlowPanel
    ...
    /**
     * A global min width, when resizing columns, to ensure, that resized columns don't get a width of 0 (means that it is not calculated at now.)
     */
    private static final int COLUMN_MIN_WIDTH = 10;
...
//    }-*/;    
    ...
    /** Fix bug, that reattaching changes calculates wrong widths. */
    public boolean forceRecalcWidth = false;
    ...
    /** Last measured width. */
    public int lastMeasuredWidth = 0;
    ...
    /**
     * Prevent two mouse events on double click.
     */
    private Element lastMouseUpTarget;
...
//        consoleLog("renderRows from: " + uidl.getIntAttribute("firstrow") + "  " + uidl.getIntAttribute("rows"));
    ...
    private void enableLazyScroller() {
        lazyScrollerIsActive = true; // Set it first, to ensure, it is in any case before disableLazyScroller sets it to false again.
    ...
    public void updatePageLength(UIDL uidl) {
        if (forceRecalcWidth)
        {
            if (totalRows > 0 && uidl.getIntAttribute("rows") > 0)
            {
                forceRecalcWidth = false;
...
//                    consoleLog("sizeInit " + firstInNewSet + "  " + (lastInNewSet - firstInNewSet));
        ...
        protected void handleCaptionEvent(Event event) {
            ...
            case Event.ONMOUSEDOWN:
                if (columnReordering
                    ...
                    dragging = true;
                    currentDragX = Integer.valueOf(WidgetUtil.getTouchOrMouseClientX(event));
                    moved = false;
            ...
            case Event.ONMOUSEMOVE:
                ...
                if (currentDragX == null || Math.abs(currentDragX.intValue()
...
//                    consoleLog("getRowHeight: " + tableHeight + "  " + rowCount + "  " + rowHeight);
                } else {
                ...
                if (enabled && !touchEventHandled) {
                    ...
                    case Event.ONMOUSEUP:
                        ...
                        if (mouseUpPreviewMatched && lastMouseDownTarget != null
                            ...
                            boolean isDoubleClick = lastMouseUpTarget == lastMouseDownTarget
                                                 && Math.abs(lastMouseEvent.getClientX() - event.getClientX()) <= 2
                                                 && Math.abs(lastMouseEvent.getClientY() - event.getClientY()) <= 2
                                                 && currentTime < lastMouseEventTime + 500;
                            
                            lastMouseUpTarget = lastMouseDownTarget;
                            lastMouseEvent = event;
                            lastMouseEventTime = currentTime;
                            
                            if (targetCellOrRowFound && !isDoubleClick) {
                                /*
...
//            consoleLog("updatePageLength: " + pageLength + "  " + rowHeight + "  " + bodyH);
...
//            consoleLog("setContainerHeight: " + containerHeight);
    ...
    public void updateHeight() {
        ...
        if (initializedAndAttached) {
//            consoleLog("updateHeight reason");
            updatePageLength();
        }
...
//        consoleLog("onScroll firstRowInView: " + firstRowInViewPort + "  postLimit: " + postLimit + "  preLimit: " + preLimit + "  lastRendered: " + lastRendered + "  firstRendered: " + firstRendered + 
//                "  scrollTop/rowHeight " + scrollTop + "  " + scrollBody.getRowHeight());
        ...
        if (postLimit <= lastRendered && preLimit >= firstRendered) {
            ...
            // a differing opinion
//            consoleLog("onScroll firstvisible: " + firstRowInViewPort);
            lastRequestedFirstvisible = firstRowInViewPort;
            client.updateVariable(paintableId, "firstvisible",
                    firstRowInViewPort, false);
            return;
...
//            consoleLog("onScroll allRenderedRowsAreNew: " + firstRowInViewPort + "  " + (pageLength * cache_rate) + "  " + (last - rowRequestHandler.getReqFirstRow() + 1));
...
//            consoleLog("onScroll preLimit < firstRendered: " + firstRowInViewPort + "  " + (pageLength * cache_rate) + "  " + (firstRendered - rowRequestHandler.getReqFirstRow()));
            rowRequestHandler.deferRowFetch();
...
//            consoleLog("onScroll postLimit > lastRendered: " + firstRowInViewPort + "  " + (pageLength * cache_rate) + "  " + lastRendered + "  " + ((firstRowInViewPort + pageLength
//                    + pageLength * cache_rate) - lastRendered));
    ...
    public boolean hasHorizontalScrollbar() {
        return scrollBody.table.getOffsetWidth() > scrollBodyPanel.getOffsetWidth();
    }
    ...
    public com.google.gwt.user.client.Element getSubPartElement(
        ...
        if (SUBPART_ROW_COL_REGEXP.test(subPart)) {
            ...
            int rowIx = Integer.valueOf(result.getGroup(1)).intValue();
            int colIx = Integer.valueOf(result.getGroup(2)).intValue();
            ...
            int headerIx = Integer.valueOf(result.getGroup(1)).intValue();
            ...
            int footerIx = Integer.valueOf(result.getGroup(1)).intValue();
