...
public class JVxSplitPane extends JSplitPane
{
    ...
    /**
     * Creates a new <code>JVxSplitPane</code> configured with the
    ...
    /**
     * Creates a new <code>JVxSplitPane</code> with the specified
     * orientation and redrawing style.
     *
     * @param pOrientation <code>JVxSplitPane.HORIZONTAL_SPLIT</code> or
     *                     <code>JVxSplitPane.VERTICAL_SPLIT</code>
     * @param pContinuousLayout  a boolean, true for the components to 
     ...
     * @param pOrientation <code>JVxSplitPane.HORIZONTAL_SPLIT</code> or
     *                     <code>JVxSplitPane.VERTICAL_SPLIT</code>
     * @param pLeftComponent the <code>Component</code> that will
     *		                 appear on the left of a horizontally-split pane, 
     *						 or at the top of a vertically-split pane
     * @param pRightComponent the <code>Component</code> that will
    ...
    public void doLayout() 
	{
		if (getDividerLocation() < 0) // Set correct initial divider position.
		{
    		...
    		{ 
    			...
    			setDividerLocation(Math.round((float)size * (float)first / (float)(first + second)) - getDividerSize() / 2);
    		}
		}
		if (getLeftComponent() != null) // if minimum size is set, use JSplitPane functionality to prevent panel getting smaller
		{
		...
		if (getRightComponent() != null) // if minimum size is set, use JSplitPane functionality to prevent panel getting smaller
		{
		...
		// Fix JSplitPanel Bug if panel was smaller than minimum size, the divider is not corrected anymore.
		int size;
		Insets ins = getInsets();
    ...
    protected void addImpl(Component pComponent, Object pConstraints, int pIndex) 
	{
		...
		else
		{
			...
			{
				// Cover component to allow panel getting smaller if minimum size is not explizitly set.
	...
	{
		// If maximum size is set ensure, that the divider does not get greater than the maximum size.
		Insets ins = getInsets();
		int size;
	...
	public void setDividerAlignment(int pDividerAlignment)
	{
		...
		{
			case DIVIDER_TOP_LEFT: setResizeWeight(0); break;
			case DIVIDER_BOTTOM_RIGHT: setResizeWeight(1); break;
			default: setResizeWeight(0.5);
		}
	}
